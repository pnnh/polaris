...
import {CodeOk, CommonResult, PLInsertResult, PLSelectResult} from "@/models/common/protocol";
...
import {articleViewerCache} from "@/worker/cache/cache";
...
export async function getArticle(request: Request, response: Response, {params}: {
...
export async function selectArticlesInChannel(request: Request, response: Response, {params}: {
    params: { channel: string, article: string }
}) {
    const domainUrl = mustGetFirstDir()
    const articleService = new SystemArticleService(domainUrl)
