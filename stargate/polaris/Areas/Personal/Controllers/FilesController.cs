...
{
    [Route("/stargate/personal/files")]
    [AllowAnonymous]
    public MSelectResult<PsFileModel> SelectFiles()
    {
...
select count(1) from ({sqlBuilder}) as temp;";
        ...
        var modelsQuery = DatabaseContextHelper.RawSqlQuery<PsFileModel>(secondContext, querySqlText, parameters);
        ...
        return new MSelectResult<PsFileModel>
        {
            Code = Codes.Ok,
            Data =
                new MSelectData<PsFileModel>
                {
                    Page = page,
                    Size = size,
                    Range = models,
