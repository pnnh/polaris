...
import {
    ApplicationLanguageProfile,
    ApplicationModel,
    ApplicationWithText
} from "@/components/common/models/application";
...
export async function queryApp(expectLang: string, appUid: string): Promise<ApplicationWithText | undefined> {
    const appList = await selectAppsFromStorage()
    const app = appList.find(app => app.uid === appUid)
    if (!app) {
        return undefined
    }
    return await getAppWithText(expectLang, app)
}

export async function getAppWithText(expectLang: string, appModel: ApplicationModel): Promise<ApplicationWithText | undefined> {
    const appWithText = appModel as ApplicationWithText
    if (!appModel.texts || typeof appModel.texts !== 'object') {
        return undefined
    }
    let targetLang = ''
    for (const key of Object.keys(appModel.texts)) {
        if (key !== expectLang && key !== langEnUS) {
            continue
        }
        const langProfile = appModel.texts[key]
        if (langProfile && targetLang !== expectLang) {
            targetLang = key
            appWithText.name = langProfile.name
            appWithText.description = langProfile.description
        }
    }
    return appWithText
}
...
interface IHomeConfig {
    files: ApplicationModel[]
}

export async function selectAppsFromStorage(): Promise<ApplicationModel[]> {
    const serverConfig = await useServerConfig()
    const storageUrl = psResolvePath(serverConfig.STORAGE_URL)
    const homeConfigPath = join(storageUrl, 'home.json5')

    const content = fs.readFileSync(homeConfigPath, 'utf8');

    const configJson5 = JSON5.parse(content) as IHomeConfig

    return configJson5.files
}

//
// export function selectApps(expectLang: string): ApplicationWithText[] {
//     return [
//         {
//             uid: articlesUid,
//             url: `/articles`,
//             mime: 'polaris/application',
