...
export function ManagementChannelsView({lang, stargateUrl, dataJson, keyword, statusFilter}: {
    lang: string
    stargateUrl: string
    dataJson: string
    keyword: string
    ...
    const channels: PSChannelModel[] = JSON.parse(dataJson);
    ...
    const allSelected = channels.length > 0 && selectedUids.size === channels.length;
    const someSelected = selectedUids.size > 0 && selectedUids.size < channels.length;

    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        setSelectedUids(e.target.checked ? new Set(channels.map(c => c.uid)) : new Set());
    };
    ...
    const handleApprove = async () => {
        if (selectedUids.size === 0) { alert('请先选择频道'); return; }
        setApproving(true);
        setApproveResult(null);
        try {
            const result = await ManagementBrowser.clientApproveChannels(stargateUrl, Array.from(selectedUids), STATUS_APPROVED);
        ...
        <Box sx={{flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column'}}>
            <div className={filterStyles.toolbar}>
                ...
                <div className={filterStyles.rightGroup}>
                    <div className={filterStyles.searchBox}>
                        <input placeholder="搜索频道..." maxLength={128} value={searchText}
            ...
            {approveResult && (
                <Alert severity={approveResult.fail === 0 ? 'success' : 'warning'} onClose={() => setApproveResult(null)} sx={{mx: 1, mt: 1}}>
                    已成功审核 {approveResult.success} 个频道{approveResult.fail > 0 && `（${approveResult.fail} 个失败）`}
                </Alert>
            )}
                ...
                <Table size="small" stickyHeader>
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell><Typography variant="body2" fontWeight={600}>频道名称</Typography></TableCell>
                    ...
                    <TableBody>
                        ...
                        {channels.map(channel => (
                            <TableRow key={channel.uid} hover selected={selectedUids.has(channel.uid)}
                                      onClick={() => !approving && handleToggle(channel.uid)}
                                      sx={{cursor: approving ? 'default' : 'pointer'}}>
                                <TableCell padding="checkbox">
                                    <Checkbox checked={selectedUids.has(channel.uid)} icon={<CheckBoxOutlineBlankIcon/>}
                                              checkedIcon={<CheckBoxIcon/>} disabled={approving}
                                              onClick={e => e.stopPropagation()} onChange={() => handleToggle(channel.uid)}/>
                                </TableCell>
                                <TableCell>
                                    <Typography variant="body2" fontWeight={500} noWrap>{channel.name}</Typography>
                                    {channel.description && (
                                ...
                                <TableCell><Typography variant="body2" color="text.secondary">{channel.owner_name || '—'}</Typography></TableCell>
                                <TableCell><StatusChip status={channel.status ?? 0}/></TableCell>
                                <TableCell>
                                    <Typography variant="body2" color="text.secondary">
                                        {channel.update_time ? new Date(channel.update_time).toLocaleString() : '—'}
                                    </Typography>
                                </TableCell>
                            </TableRow>
                        ))}
