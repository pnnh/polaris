...
export function ManagementChannelsView({ lang, stargateUrl, dataJson, keyword, statusFilter }: {
    ...
    const handleApprove = async () => {
        ...
        try {
            const result = await ManagementBrowser.clientApproveChannels(stargateUrl, Array.from(selectedUids), 1);
                ...
                <table className="w-full text-sm border-collapse">
                    <thead className="sticky top-0 bg-background">
                        <tr>
                            ...
                            <th className="p-2 border-b text-left font-semibold">频道名称</th>
                    ...
                    <tbody>
                        ...
                        {channels.map(channel => (
                            <tr key={channel.uid}
                                data-selected={selectedUids.has(channel.uid) || undefined}
                                ...
                                <td className="w-12 p-1">
                                    <InlineCheckbox checked={selectedUids.has(channel.uid)}
                                        disabled={approving}
                                        onClick={e => e.stopPropagation()}
                                        onChange={() => handleToggle(channel.uid)} />
                                </td>
                                ...
                                <td className="w-40 p-2 text-muted-foreground text-xs">{channel.owner_name || '—'}</td>
                                <td className="w-28 p-2"><StatusBadge status={channel.status ?? 0} /></td>
